...
class MetalProblem:
    ...
    def score(self):
        ...
        metal_py = convert_source_to_py(
            self.metalKernel.header + self.metalKernel.source
        )
            ...
            for grid_y in range(self.grid[1]):
                ...
                for name, value in local_variables.items():
                    if isinstance(value, ThreadgroupMemory):
                        full["shared_reads"] = max(
                            locals()[name].get_reads(), full["shared_reads"]
                        )
                        full["shared_writes"] = max(
                            locals()[name].get_writes(), full["shared_writes"]
                        )
...
class TrackedArray:
    ...
    def __getitem__(self, index):
        if index > len(self.data):
            raise Exception(
                f"Out-of-bounds access in mx.array '{self.name}' at index {index}."
            )
...
class ThreadgroupMemory:
    ...
    def __getitem__(self, index):
        if index > len(self.data):
            raise Exception(
                f"Out-of-bounds access in ThreadgroupMemory at index {index}."
            )
        self.reads_per_thread += 1
        return self.data[index]
...
def preprocess_source(source):
    ...
    metal_source = re.sub(
        r"threadgroup float (\w+)\[(\w+)\]\[(\w+)\];",
        r"\1 = ThreadgroupMemory(\2, \3)",
        metal_source,
    )
    metal_source = re.sub(
        r"threadgroup float (\w+)\[(\w+)\];",
        r"\1 = ThreadgroupMemory(\2)",
        metal_source,
    )
    metal_source = re.sub(
        r"threadgroup_barrier\(mem_flags::mem_threadgroup\);", "", metal_source
    )
    metal_source = re.sub(r"metal::", "", metal_source)
    metal_source = re.sub(
        r"\b(uint|int|float|double|auto|constant)\b", "", metal_source
    )
